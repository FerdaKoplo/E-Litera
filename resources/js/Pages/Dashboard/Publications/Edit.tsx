...
const Edit: React.FC<Props> = ({ categories, locations, publication }) => {
    ...
    const [pdfFileName, setPdfFileName] = useState<string | null>(publication.pdf_url || null)
        ...
        <DashboardLayout header={<h2 className="font-semibold text-2xl text-gray-800">Edit Publication</h2>} breadcrumbs={breadcrumbs}>
            ...
            <form onSubmit={submit} className="space-y-10 w-full">
                <div className="flex gap-10 w-full max-w-5xl">
                    <div className="flex-1 max-w-xl space-y-6">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Input id="author" name="author" value={data.author} onChange={e => setData('author', e.target.value)}
                                className="w-full bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-fuchsia-400" isFocused={false} />
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Input id="publication_description" name="publication_description"
                                value={data.publication_description} onChange={e => setData('publication_description', e.target.value)}
                                className="w-full bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-fuchsia-400" isFocused={false} />
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Select value={data.type} onValueChange={value => setData('type', value.toLowerCase())}>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Select value={data.location_id !== null ? String(data.location_id) : ""} onValueChange={value => setData('location_id', Number(value))}>
                                <SelectTrigger className="w-full bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-fuchsia-400">
                    ...
                    {/* Right Column: PDF + Image */}
                    <div className="flex-1 max-w-xl space-y-6">

                        {/* PDF Upload */}
                        <div className="flex flex-col gap-2">
                            <Label forInput="pdf_url" value="PDF File (for Ebook/Journal)" />
                            <div {...getPdfRootProps()} className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition ${isPdfDragActive ? "border-fuchsia-500 bg-fuchsia-50" : "border-gray-300"}`}>
                                ...
                                {pdfFileName ? (
                                    ...
                                    <p className="text-sm text-gray-500">{isPdfDragActive ? "Drop the PDF here..." : "Drag & drop or click to upload PDF"}</p>
                                )}
                            </div>
                            {uploadProgress > 0 && (
                                <Progress
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div {...getImageRootProps()} className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition ${isImageDragActive ? "border-fuchsia-500 bg-fuchsia-50" : "border-gray-300"}`}>
                                ...
                                {previewImage ? (
                                    ...
                                    <p className="text-sm text-gray-500">{isImageDragActive ? "Drop the image here..." : "Drag & drop or click to upload cover image"}</p>
                                )}
                            </div>
                            {uploadProgress > 0 && (
                                <Progress
                ...
                <div className="flex items-center gap-4">
                    <Button type="button" process={processing} className="px-6 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 transition" onClick={saveDraft}>
                        Save Draft
                    </Button>
                    <Button type="submit" process={processing} className="px-6 py-2 rounded-lg bg-black text-white hover:bg-fuchsia-400 transition">
                        Update
                    </Button>
                    <Link href={route('publications.index')} className="text-gray-600">Cancel</Link>
                </div>
            </form>
        </DashboardLayout>
    )
